...
@section('content')
    <div class="mb-6"><h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Create Module Offering') }}
        </h2></div>

    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('academic-setup.module-offerings.store') }}" method="POST">
                        @csrf
                        
                        <!-- Note: Ideally use Livewire or AJAX to filter modules by program, but standard select for now -->
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                <label for="module_id" class="block text-sm font-medium text-gray-700">Module</label>
                                <select name="module_id" id="module_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 select2" required>
                            ...
                            <div>
                                ...
                                <select name="nta_level" id="nta_level" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    <option value="4" {{ old('nta_level') == 4 ? 'selected' : '' }}>NTA Level 4</option>
                                    <option value="5" {{ old('nta_level') == 5 ? 'selected' : '' }}>NTA Level 5</option>
                                    <option value="6" {{ old('nta_level') == 6 ? 'selected' : '' }}>NTA Level 6</option>
                                </select>
                        ...
                        <div class="mt-6 flex justify-end">
                            ...
                            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">Save Offering</button>
                        </div>
                    </form>
                </div>
            </div>
