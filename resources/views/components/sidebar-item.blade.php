@props(['active' => false, 'icon' => null, 'label' => '', 'route' => null, 'children' => []])
...
<div x-data="{ open: {{ $isActive ? 'true' : 'false' }} }">
    <a 
        ...
        @endif
        class="flex items-center px-6 py-3 text-gray-100 hover:bg-blue-800 transition-colors duration-200 {{ $isActive && !$hasChildren ? 'bg-blue-800 border-l-4 border-yellow-400' : '' }}"
    >
        @if($icon)
            <span class="mr-3">
                {{-- Dynamic Icon Rendering using Heroicons (outline) --}}
                @switch($icon)
                    @case('home')
    ...
    @if($hasChildren)
        <div x-show="open" x-collapse class="bg-blue-950/50">
            @foreach($children as $child)
                <a href="{{ isset($child['route']) && $child['route'] !== '#' ? route($child['route']) : '#' }}" 
                   class="flex items-center pl-14 py-2 text-sm text-gray-300 hover:text-white hover:bg-blue-800 transition-colors duration-200 {{ request()->routeIs($child['route']) ? 'text-yellow-400 font-semibold' : '' }}">
                    @if(isset($child['icon']))
                         <span class="mr-2">
                             {{-- Simplified icon logic for children, can repeat switch if needed, but keeping simple for now --}}
                              @switch($child['icon'])
                                @case('bookmark')
